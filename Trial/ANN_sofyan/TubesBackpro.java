...
public class TubesBackpro {
    ...
    public static void main(String[] args) throws SQLException {
        ...
        while (epoch< 1000) {
            ...
            while (rs.next()) {
                //datake=datake+1;
                //System.out.println("datake "+datake);
                ...
                for (int i = 0; i < y.length; i++) {
                    y[i] = ((x1 * arrW1[i]) + (x2 * arrW2[i]) + (x3 * arrW3[i]))+b1[i]; //summing function
                    fy[i] = 1 / (1 + Math.exp(-y[i])); //fungsi aktifasi
                ...
                //hitung DW , DB di input
                for (int i = 0; i < y.length; i++) {
                    arrDw1[i] = lr * D1[i] * x1;
                    arrDw2[i] = lr * D1[i] * x2;
                    arrDw3[i] = lr * D1[i] * x3;
                    arrDb1[i] = lr * D1[i]; //DB di input
                    arrDw4[i] = lr * D2 * fy[i]; //hitung DW di output
                }
                //hitung Db di output
                Db2 = lr * D2;
        ...
        double akurasi = 0;
        int jumPolaTest = 0;
        System.out.println("--HASIL TESTING DATA TRAINING--");
        while (rs.next()) {
            ekuadrat = 0;
        ...
        System.out.println("--HASIL TESTING DATA TESTING--");

        s = "select * from datatest";
        rs = db.getData(s);
        jumBenar = 0;
        akurasi = 0;
        jumPolaTest = 0;
